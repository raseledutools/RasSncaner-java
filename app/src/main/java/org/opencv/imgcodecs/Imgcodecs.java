...
public class Imgcodecs {
  ...
  public static final int IMWRITE_PNG_FILTER_NONE = 8,
      ...
      IMWRITE_PNG_FAST_FILTERS =
          (IMWRITE_PNG_FILTER_NONE | IMWRITE_PNG_FILTER_SUB | IMWRITE_PNG_FILTER_UP),
      IMWRITE_PNG_ALL_FILTERS =
          (IMWRITE_PNG_FAST_FILTERS | IMWRITE_PNG_FILTER_AVG | IMWRITE_PNG_FILTER_PAETH);
  ...
  public static final int IMWRITE_TIFF_RESOLUTION_UNIT_NONE = 1,
   ...
   * <p>imread
   *
   * <p>The {@code imread} function loads an image from the specified file and returns OpenCV
   * matrix. If the image cannot be read (because of a missing file, improper permissions, or
   * unsupported/invalid format), the function returns an empty matrix.
   *
   * <p>Currently, the following file formats are supported:
   *
   * <ul>
   *   <li>Windows bitmaps - \*.bmp, \*.dib (always supported)
   *   <li>GIF files - \*.gif (always supported)
   ...
   *   <li>AVIF - \*.avif (see the *Note* section)
   *   <li>Portable image format - \*.pbm, \*.pgm, \*.ppm, \*.pxm, \*.pnm (always supported)
   *   <li>PFM files - \*.pfm (see the *Note* section)
   ...
   *
   * <ul>
   *   <li>The function determines the type of an image by its content, not by the file extension.
   ...
   *       available. Results may differ from the output of cvtColor().
   *   <li>On Microsoft Windows\* and Mac OS\*, the codecs shipped with OpenCV (libjpeg, libpng,
   *       libtiff, and libjasper) are used by default. So, OpenCV can always read JPEGs, PNGs, and
   *       TIFFs. On Mac OS, there is also an option to use native Mac OS image readers. However,
   *       beware that currently these native image loaders give images with different pixel values
   *       because of the color management embedded into Mac OS.
   *   <li>On Linux\*, BSD flavors, and other Unix-like open-source operating systems, OpenCV looks
   *       for codecs supplied with the OS. Ensure the relevant packages are installed (including
   *       development files, such as "libjpeg-dev" in Debian\* and Ubuntu\*) to get codec support,
   *       or turn on the OPENCV_BUILD_3RDPARTY_LIBS flag in CMake.
   *   <li>If the *WITH_GDAL* flag is set to true in CMake and REF: IMREAD_LOAD_GDAL is used to load
   *       the image, the [GDAL](http://www.gdal.org) driver will be used to decode the image,
   *       supporting [Raster](http://www.gdal.org/formats_list.html) and
   *       [Vector](http://www.gdal.org/ogr_formats.html) formats.
   *   <li>If EXIF information is embedded in the image file, the EXIF orientation will be taken
   *       into account, and thus the image will be rotated accordingly unless the flags REF:
   *       IMREAD_IGNORE_ORIENTATION or REF: IMREAD_UNCHANGED are passed.
   *   <li>Use the IMREAD_UNCHANGED flag to preserve the floating-point values from PFM images.
   *   <li>By default, the number of pixels must be less than 2^30. This limit can be changed by
   *       setting the environment variable {@code OPENCV_IO_MAX_IMAGE_PIXELS}. See REF:
   *       tutorial_env_reference.
   * </ul>
   *
   ...
   * function only in what argument(s) it accepts and the return value.
   *
   * @param filename Name of file to be loaded.
   * @param dst object in which the image will be loaded.
   * @param flags Flag that can take values of cv::ImreadModes, default with cv::IMREAD_COLOR_BGR.
   *     <b>Note:</b> The image passing through the img parameter can be pre-allocated. The memory
   *     is reused if the shape and the type match with the load image.
   */
   ...
   *
   * @param filename Name of file to be loaded.
   * @param dst object in which the image will be loaded. <b>Note:</b> The image passing through the
   *     img parameter can be pre-allocated. The memory is reused if the shape and the type match
   *     with the load image.
   */
  ...
  //
  // C++:  Mat cv::imreadWithMetadata(String filename, vector_int& metadataTypes, vector_Mat&
  // metadata, int flags = IMREAD_ANYCOLOR)
  //

  /**
   * Reads an image from a file along with associated metadata.
   *
   * <p>This function behaves similarly to cv::imread(), loading an image from the specified file.
   ...
   *
   * @param filename Name of the file to be loaded.
   * @param metadataTypes Output vector with types of metadata chunks returned in metadata, see
   *     ImageMetadataType.
   * @param metadata Output vector of vectors or vector of matrices to store the retrieved metadata.
   * @param flags Flag that can take values of cv::ImreadModes, default with cv::IMREAD_ANYCOLOR.
   * @return The loaded image as a cv::Mat object. If the image cannot be read, the function returns
   *     an empty matrix.
   */
  public static Mat imreadWithMetadata(
      String filename, MatOfInt metadataTypes, List<Mat> metadata, int flags) {
    Mat metadataTypes_mat = metadataTypes;
    Mat metadata_mat = new Mat();
    Mat retVal =
        new Mat(
            imreadWithMetadata_0(
                filename, metadataTypes_mat.nativeObj, metadata_mat.nativeObj, flags));
   ...
   */
  public static Mat imreadWithMetadata(
      String filename, MatOfInt metadataTypes, List<Mat> metadata) {
    Mat metadataTypes_mat = metadataTypes;
    Mat metadata_mat = new Mat();
    Mat retVal =
        new Mat(
            imreadWithMetadata_1(filename, metadataTypes_mat.nativeObj, metadata_mat.nativeObj));
    Converters.Mat_to_vector_Mat(metadata_mat, metadata);
    metadata_mat.release();
    return retVal;
  }
  ...
  //
  // C++:  bool cv::imreadmulti(String filename, vector_Mat& mats, int start, int count, int flags =
  // IMREAD_ANYCOLOR)
  //
   ...
   *
   * <p>The function imreadmulti loads a specified range from a multi-page image from the specified
   ...
   * @return automatically generated
   */
  public static boolean imreadmulti(
      String filename, List<Mat> mats, int start, int count, int flags) {
    Mat mats_mat = new Mat();
  ...
  //
  // C++:  bool cv::imreadanimation(String filename, Animation& animation, int start = 0, int count
  // = INT16_MAX)
  //
   ...
   *
   * <p>The function imreadanimation loads frames from an animated image file (e.g., GIF, AVIF,
   * APNG, WEBP) into the provided Animation struct.
   *
   * @param filename A string containing the path to the file.
   * @param animation A reference to an Animation structure where the loaded frames will be stored.
   *     It should be initialized before the function is called.
   ...
   * @return Returns true if the file was successfully loaded and frames were extracted; returns
   *     false otherwise.
   */
  public static boolean imreadanimation(
      String filename, Animation animation, int start, int count) {
  ...
  //
  // C++:  bool cv::imdecodeanimation(Mat buf, Animation& animation, int start = 0, int count =
  // INT16_MAX)
  //
   ...
   *
   * <p>The function imdecodeanimation loads frames from an animated image buffer (e.g., GIF, AVIF,
   * APNG, WEBP) into the provided Animation struct.
   *
   ...
   * @param start The index of the first frame to load. This is optional and defaults to 0.
   * @param count The number of frames to load. This is optional and defaults to 32767.
   * @return Returns true if the buffer was successfully loaded and frames were extracted; returns
   *     false otherwise.
   */
  ...
  //
  // C++:  bool cv::imwriteanimation(String filename, Animation animation, vector_int params =
  // std::vector<int>())
  //
   ...
   *
   * <p>The function imwriteanimation saves the provided Animation data to the specified file in an
   * animated format. Supported formats depend on the implementation and may include formats like
   * GIF, AVIF, APNG, or WEBP.
   *
   * @param filename The name of the file where the animation will be saved. The file extension
   *     determines the format.
   * @param animation A constant reference to an Animation struct containing the frames and metadata
   *     to be saved.
   * @param params Optional format-specific parameters encoded as pairs (paramId_1, paramValue_1,
   *     paramId_2, paramValue_2, ...). These parameters are used to specify additional options for
   *     the encoding process. Refer to {@code cv::ImwriteFlags} for details on possible parameters.
   ...
   *     determines the format.
   * @param animation A constant reference to an Animation struct containing the frames and metadata
   *     to be saved. These parameters are used to specify additional options for the encoding
  ...
  //
  // C++:  bool cv::imencodeanimation(String ext, Animation animation, vector_uchar& buf, vector_int
   ...
   *
   * <p>The function imencodeanimation encodes the provided Animation data into a memory buffer in
   * an animated format. Supported formats depend on the implementation and may include formats like
   * GIF, AVIF, APNG, or WEBP.
   *
   ...
   */
  public static boolean imencodeanimation(
      String ext, Animation animation, MatOfByte buf, MatOfInt params) {
    Mat buf_mat = buf;
    Mat params_mat = params;
    return imencodeanimation_0(
        ext, animation.getNativeObjAddr(), buf_mat.nativeObj, params_mat.nativeObj);
  }
   ...
   *     to be encoded.
   * @param buf A reference to a vector of unsigned chars where the encoded data will be stored.
   *     paramValue_1, paramId_2, paramValue_2, ...). These parameters are used to specify
   *     additional options for the encoding process. Refer to {@code cv::ImwriteFlags} for details
   ...
   *
   * <p>The function imcount returns the number of pages in a multi-page image (e.g. TIFF), the
   * number of frames in an animation (e.g. AVIF), and 1 otherwise. If the image cannot be decoded,
   * 0 is returned.
   *
   * @param filename Name of file to be loaded.
   * @param flags Flag that can take values of cv::ImreadModes, default with cv::IMREAD_ANYCOLOR.
   *     TODO: when cv::IMREAD_LOAD_GDAL flag used the return value will be 0 or 1 because OpenCV's
   *     GDAL decoder doesn't support multi-page reading yet.
   * @return automatically generated
   */
   ...
   * 0 is returned.
   *
   * @param filename Name of file to be loaded. TODO: when cv::IMREAD_LOAD_GDAL flag used the return
   *     value will be 0 or 1 because OpenCV's GDAL decoder doesn't support multi-page reading yet.
   * @return automatically generated
   */
   ...
   *
   * <p>The function imwrite saves the image to the specified file. The image format is chosen based
   * on the filename extension (see cv::imread for the list of extensions). In general, only 8-bit
   * unsigned (CV_8U) single-channel or 3-channel (with 'BGR' channel order) images can be saved
   * using this function, with these exceptions:
   *
   * <ul>
   *   <li>With BMP encoder, 8-bit unsigned (CV_8U) images can be saved.
   *       <ul>
   *         <li>BMP images with an alpha channel can be saved using this function. To achieve this,
   *             create an 8-bit 4-channel (CV_8UC4) BGRA image, ensuring the alpha channel is the
   *             last component. Fully transparent pixels should have an alpha value of 0, while
   *             fully opaque pixels should have an alpha value of 255. OpenCV v4.13.0 or later use
   *             BI_BITFIELDS compression as default. See IMWRITE_BMP_COMPRESSION.
   *       </ul>
   *   <li>With OpenEXR encoder, only 32-bit float (CV_32F) images can be saved. More than 4
   *       channels can be saved. (imread can load it then.)
   *       <ul>
   *         <li>8-bit unsigned (CV_8U) images are not supported.
   *       </ul>
   *   <li>With Radiance HDR encoder, non 64-bit float (CV_64F) images can be saved.
   *       <ul>
   *         <li>All images will be converted to 32-bit float (CV_32F).
   *       </ul>
   *   <li>With JPEG 2000 encoder, 8-bit unsigned (CV_8U) and 16-bit unsigned (CV_16U) images can be
   *       saved.
   *   <li>With JPEG XL encoder, 8-bit unsigned (CV_8U), 16-bit unsigned (CV_16U) and 32-bit
   *       float(CV_32F) images can be saved.
   *       <ul>
   *         <li>JPEG XL images with an alpha channel can be saved using this function. To achieve
   *             this, create an 8-bit 4-channel (CV_8UC4) / 16-bit 4-channel (CV_16UC4) / 32-bit
   *             float 4-channel (CV_32FC4) BGRA image, ensuring the alpha channel is the last
   *             component. Fully transparent pixels should have an alpha value of 0, while fully
   *             opaque pixels should have an alpha value of 255/65535/1.0.
   *       </ul>
   *   <li>With PAM encoder, 8-bit unsigned (CV_8U) and 16-bit unsigned (CV_16U) images can be
   *       saved.
   *   <li>With PNG encoder, 8-bit unsigned (CV_8U) and 16-bit unsigned (CV_16U) images can be
   *       saved.
   *       <ul>
   *         <li>PNG images with an alpha channel can be saved using this function. To achieve this,
   *             create an 8-bit 4-channel (CV_8UC4) / 16-bit 4-channel (CV_16UC4) BGRA image,
   *             ensuring the alpha channel is the last component. Fully transparent pixels should
   *             have an alpha value of 0, while fully opaque pixels should have an alpha value of
   *             255/65535(see the code sample below).
   *       </ul>
   *   <li>With PGM/PPM encoder, 8-bit unsigned (CV_8U) and 16-bit unsigned (CV_16U) images can be
   *       saved.
   *   <li>With TIFF encoder, 8-bit unsigned (CV_8U), 8-bit signed (CV_8S), 16-bit unsigned
   *       (CV_16U), 16-bit signed (CV_16S), 32-bit signed (CV_32S), 32-bit float (CV_32F) and
   *       64-bit float (CV_64F) images can be saved.
   *       <ul>
   *         <li>Multiple images (vector of Mat) can be saved in TIFF format (see the code sample
   *             below).
   *         <li>32-bit float 3-channel (CV_32FC3) TIFF images will be saved using the LogLuv high
   *             dynamic range encoding (4 bytes per pixel)
   *       </ul>
   *   <li>With GIF encoder, 8-bit unsigned (CV_8U) images can be saved.
   *       <ul>
   *         <li>GIF images with an alpha channel can be saved using this function. To achieve this,
   *             create an 8-bit 4-channel (CV_8UC4) BGRA image, ensuring the alpha channel is the
   *             last component. Fully transparent pixels should have an alpha value of 0, while
   *             fully opaque pixels should have an alpha value of 255.
   *         <li>8-bit single-channel images (CV_8UC1) are not supported due to GIF's limitation to
   *             indexed color formats.
   *       </ul>
   *   <li>With AVIF encoder, 8-bit unsigned (CV_8U) and 16-bit unsigned (CV_16U) images can be
   *       saved.
   *       <ul>
   *         <li>CV_16U images can be saved as only 10-bit or 12-bit (not 16-bit). See
   *             IMWRITE_AVIF_DEPTH.
   *         <li>AVIF images with an alpha channel can be saved using this function. To achieve
   *             this, create an 8-bit 4-channel (CV_8UC4) / 16-bit 4-channel (CV_16UC4) BGRA image,
   *             ensuring the alpha channel is the last component. Fully transparent pixels should
   *             have an alpha value of 0, while fully opaque pixels should have an alpha value of
   *             255 (8-bit) / 1023 (10-bit) / 4095 (12-bit) (see the code sample below).
   *       </ul>
   *       If the image format is not supported, the image will be converted to 8-bit unsigned
   *       (CV_8U) and saved that way.
   * </ul>
   *
   * If the format, depth or channel order is different, use Mat::convertTo and cv::cvtColor to
   * convert it before saving. Or, use the universal FileStorage I/O functions to save the image to
   * XML or YAML format.
   *
   * <p>The sample below shows how to create a BGRA image, how to set custom compression parameters
   * and save it to a PNG file. It also demonstrates how to save multiple images in a TIFF file:
  ...
  //
  // C++:  bool cv::imwriteWithMetadata(String filename, Mat img, vector_int metadataTypes,
  // vector_Mat metadata, vector_int params = std::vector<int>())
  //

  /**
   * Saves an image to a specified file with metadata
   *
   * <p>The function imwriteWithMetadata saves the image to the specified file. It does the same
   * thing as imwrite, but additionally writes metadata if the corresponding format supports it.
   *
   * @param filename Name of the file. As with imwrite, image format is determined by the file
   *     extension.
   * @param img (Mat or vector of Mat) Image or Images to be saved.
   * @param metadataTypes Vector with types of metadata chucks stored in metadata to write, see
   *     ImageMetadataType.
   * @param metadata Vector of vectors or vector of matrices with chunks of metadata to store into
   *     the file
   * @param params Format-specific parameters encoded as pairs (paramId_1, paramValue_1, paramId_2,
   *     paramValue_2, ... .) see cv::ImwriteFlags
   * @return automatically generated
  ...
  public static boolean imwriteWithMetadata(
      String filename, Mat img, MatOfInt metadataTypes, List<Mat> metadata, MatOfInt params) {
    Mat metadataTypes_mat = metadataTypes;
   ...
   */
  public static boolean imwriteWithMetadata(
      String filename, Mat img, MatOfInt metadataTypes, List<Mat> metadata) {
    Mat metadataTypes_mat = metadataTypes;
    Mat metadata_mat = Converters.vector_Mat_to_Mat(metadata);
    return imwriteWithMetadata_1(
        filename, img.nativeObj, metadataTypes_mat.nativeObj, metadata_mat.nativeObj);
  }
   ...
   *
   * <p>The function imdecode reads an image from the specified buffer in the memory. If the buffer
   * is too short or contains invalid data, the function returns an empty matrix ( Mat::data==NULL
   * ).
   *
  ...
  //
  // C++:  Mat cv::imdecodeWithMetadata(Mat buf, vector_int& metadataTypes, vector_Mat& metadata,
   ...
   *
   * <p>This function decodes an image from the specified memory buffer. If the buffer is too short
   * or contains invalid data, the function returns an empty matrix ( Mat::data==NULL ).
   *
   * <p>See cv::imread for the list of supported formats and flags description.
   *
   * <p><b>Note:</b> In the case of color images, the decoded images will have the channels stored
   ...
   * @param metadataTypes Output vector with types of metadata chucks returned in metadata, see
   *     cv::ImageMetadataType
   * @param metadata Output vector of vectors or vector of matrices to store the retrieved metadata
   * @param flags Flag that can take values of cv::ImreadModes, default with cv::IMREAD_ANYCOLOR.
   * @return The decoded image as a cv::Mat object. If decoding fails, the function returns an empty
   *     matrix.
   */
  public static Mat imdecodeWithMetadata(
      Mat buf, MatOfInt metadataTypes, List<Mat> metadata, int flags) {
    Mat metadataTypes_mat = metadataTypes;
    Mat metadata_mat = new Mat();
    Mat retVal =
        new Mat(
            imdecodeWithMetadata_0(
                buf.nativeObj, metadataTypes_mat.nativeObj, metadata_mat.nativeObj, flags));
    Converters.Mat_to_vector_Mat(metadata_mat, metadata);
    metadata_mat.release();
    return retVal;
  }
  ...
  public static Mat imdecodeWithMetadata(Mat buf, MatOfInt metadataTypes, List<Mat> metadata) {
    ...
    Mat retVal =
        new Mat(
            imdecodeWithMetadata_1(
                buf.nativeObj, metadataTypes_mat.nativeObj, metadata_mat.nativeObj));
    Converters.Mat_to_vector_Mat(metadata_mat, metadata);
    metadata_mat.release();
    return retVal;
  }
   ...
   *
   * <p>The function imdecodemulti reads a multi-page image from the specified buffer in the memory.
   * If the buffer is too short or contains invalid data, the function returns false.
   *
   * <p>See cv::imreadmulti for the list of supported formats and flags description.
   *
   * <p><b>Note:</b> In the case of color images, the decoded images will have the channels stored
   * in <b>B G R</b> order.
   *
  ...
  public static boolean imdecodemulti(Mat buf, int flags, List<Mat> mats, Range range) {
    Mat mats_mat = new Mat();
    boolean retVal =
        imdecodemulti_0(buf.nativeObj, flags, mats_mat.nativeObj, range.start, range.end);
    Converters.Mat_to_vector_Mat(mats_mat, mats);
    mats_mat.release();
    return retVal;
  }
  ...
  //
  // C++:  bool cv::imencodeWithMetadata(String ext, Mat img, vector_int metadataTypes, vector_Mat
  // metadata, vector_uchar& buf, vector_int params = std::vector<int>())
  //
   ...
   */
  public static boolean imencodeWithMetadata(
      String ext, Mat img, MatOfInt metadataTypes, List<Mat> metadata, MatOfByte buf) {
    Mat metadataTypes_mat = metadataTypes;
    Mat metadata_mat = Converters.vector_Mat_to_Mat(metadata);
    Mat buf_mat = buf;
    return imencodeWithMetadata_1(
        ext, img.nativeObj, metadataTypes_mat.nativeObj, metadata_mat.nativeObj, buf_mat.nativeObj);
  }
   ...
   *
   * <p>The function is analog to cv::imencode for in-memory multi-page image compression. See
   ...
   *
   * <p>The function haveImageReader checks if OpenCV is capable of reading the specified file. This
   * can be useful for verifying support for a given image format before attempting to load an
   * image.
   *
   * @param filename The name of the file to be checked.
   * @return true if an image reader for the specified file is available and the file can be opened,
   *     false otherwise.
   *     <p><b>Note:</b> The function checks the availability of image codecs that are either built
   *     into OpenCV or dynamically loaded. It does not load the image codec implementation and
   *     decode data, but uses signature check. If the file cannot be opened or the format is
   *     unsupported, the function will return false.
   *     <p>SEE: cv::haveImageWriter, cv::imread, cv::imdecode
   */
   ...
   *
   * <p>The function haveImageWriter checks if OpenCV is capable of writing images with the
   * specified file extension. This can be useful for verifying support for a given image format
   * before attempting to save an image.
   *
   * @param filename The name of the file or the file extension (e.g., ".jpg", ".png"). It is
   *     recommended to provide the file extension rather than the full file name.
   * @return true if an image writer for the specified extension is available, false otherwise.
   *     <p><b>Note:</b> The function checks the availability of image codecs that are either built
   *     into OpenCV or dynamically loaded. It does not check for the actual existence of the file
   *     but rather the ability to write files of the given type.
   *     <p>SEE: cv::haveImageReader, cv::imwrite, cv::imencode
   */
  ...
  private static native long imreadWithMetadata_0(
      String filename, long metadataTypes_mat_nativeObj, long metadata_mat_nativeObj, int flags);

  private static native long imreadWithMetadata_1(
      String filename, long metadataTypes_mat_nativeObj, long metadata_mat_nativeObj);
  ...
  private static native boolean imreadmulti_2(
      String filename, long mats_mat_nativeObj, int start, int count, int flags);

  private static native boolean imreadmulti_3(
      String filename, long mats_mat_nativeObj, int start, int count);

  // C++:  bool cv::imreadanimation(String filename, Animation& animation, int start = 0, int count
  // = INT16_MAX)
  private static native boolean imreadanimation_0(
      String filename, long animation_nativeObj, int start, int count);

  private static native boolean imreadanimation_1(
      String filename, long animation_nativeObj, int start);
  ...
  private static native boolean imdecodeanimation_0(
      long buf_nativeObj, long animation_nativeObj, int start, int count);

  private static native boolean imdecodeanimation_1(
      long buf_nativeObj, long animation_nativeObj, int start);
  ...
  private static native boolean imwriteanimation_0(
      String filename, long animation_nativeObj, long params_mat_nativeObj);
  ...
  private static native boolean imencodeanimation_0(
      String ext, long animation_nativeObj, long buf_mat_nativeObj, long params_mat_nativeObj);

  private static native boolean imencodeanimation_1(
      String ext, long animation_nativeObj, long buf_mat_nativeObj);
  ...
  private static native long imdecodeWithMetadata_0(
      long buf_nativeObj, long metadataTypes_mat_nativeObj, long metadata_mat_nativeObj, int flags);

  private static native long imdecodeWithMetadata_1(
      long buf_nativeObj, long metadataTypes_mat_nativeObj, long metadata_mat_nativeObj);

  // C++:  bool cv::imdecodemulti(Mat buf, int flags, vector_Mat& mats, Range range = Range::all())
  private static native boolean imdecodemulti_0(
      long buf_nativeObj, int flags, long mats_mat_nativeObj, int range_start, int range_end);

  private static native boolean imdecodemulti_1(
      long buf_nativeObj, int flags, long mats_mat_nativeObj);
  ...
  private static native boolean imencodemulti_0(
      String ext, long imgs_mat_nativeObj, long buf_mat_nativeObj, long params_mat_nativeObj);

  private static native boolean imencodemulti_1(
      String ext, long imgs_mat_nativeObj, long buf_mat_nativeObj);
