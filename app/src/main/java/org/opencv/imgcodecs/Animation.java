...
// C++: class Animation
/**
 * Represents an animation with multiple frames. The {@code Animation} struct is designed to store
 * and manage data for animated sequences such as those from animated formats (e.g., GIF, AVIF,
 * APNG, WebP). It provides support for looping, background color settings, frame timing, and frame
 * storage.
 */
public class Animation {
   ...
   *           continuously.
   *       <li>Positive values denote finite repeat counts, allowing the animation to play a limited
   *           number of times.
   *       <li>If a negative value or a value beyond the maximum of {@code 0xffff} (65535) is
   *           provided, it is reset to {@code 0} (infinite looping) to maintain valid bounds.
   *     </ul>
   *
   * @param bgColor A {@code Scalar} object representing the background color in BGR format:
   *     <ul>
   *       <li>Defaults to {@code Scalar()}, indicating an empty color (usually transparent if
   *           supported).
   *       <li>This background color provides a solid fill behind frames that have transparency,
   *           ensuring a consistent display appearance.
   *     </ul>
   */
  public Animation(int loopCount, Scalar bgColor) {
    nativeObj =
        Animation_0(loopCount, bgColor.val[0], bgColor.val[1], bgColor.val[2], bgColor.val[3]);
  }
   ...
   *   <li>Positive values denote finite repeat counts, allowing the animation to play a limited
   *       number of times.
   *   <li>If a negative value or a value beyond the maximum of {@code 0xffff} (65535) is provided,
   *       it is reset to {@code 0} (infinite looping) to maintain valid bounds.
   * </ul>
   *
   * <ul>
   *   <li>Defaults to {@code Scalar()}, indicating an empty color (usually transparent if
