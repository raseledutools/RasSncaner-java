...
public class PdfCreator {
  ...
  /**
   * Loads a list of PDF fonts with fallback mechanisms based on provided font files. The method
   * attempts to load specified fonts from the application's assets. If none of the specified fonts
   * are found or successfully loaded, it falls back to a default font configuration, using
  ...
  private static List<PDFont> loadFontsWithFallbacks(PDDocument document, Context context) {
    ...
    String[] candidates =
        new String[] {
          ...
          "fonts/NotoSansCJKsc-Regular.ttf", // CJK (Han) – one font is sufficient for the invisible
          // layer
  ...
  /**
   * Copies a file from the assets directory to the cache directory. If the file already exists in
   * the cache directory and has a non-zero length, it is returned as-is. Otherwise, the file is
   * copied from the assets directory to the cache directory.
   *
  ...
  /**
   * Writes text to a PDF page content stream with support for font fallbacks. Ensures that the text
   * is displayed using the most suitable font from the provided list of fonts. If no suitable font
   * is found for a character, it substitutes the character with the Unicode replacement character
   * (�).
   *
   ...
   * @param fonts a {@link List} of {@link PDFont} objects to try for displaying the text. The order
   *     of fonts in the list determines the priority of fallback.
   ...
   * correctly.
   *
   * <p>OCR engines like Tesseract may return RTL text in visual order (as it appears on screen from
   * left to right), but PDF text layers need logical order (the order in which characters are
   * read/typed).
   *
