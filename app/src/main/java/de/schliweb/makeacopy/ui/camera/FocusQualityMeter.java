...
/**
 * Pure-Java normalization and segment mapping for the live focus-quality (sharpness) indicator
 * (see {@code docs/focus_quality_indicator_design.md}).
 *
 ...
 *       the indicator cannot become permanently stuck at a stale peak after the scene changes
 *       (e.g. moving from a text-dense page to a sparse one).
 ...
 * </ul>
 *
 * <p>This class is deliberately free of Android and OpenCV dependencies so it can be unit-tested
 * on the JVM.
 */
public final class FocusQualityMeter {
   ...
   * @param nowMs monotonic-ish timestamp in milliseconds (e.g. {@code
   *     System.currentTimeMillis()}); used only for decay deltas
  ...
  public double update(double rawVariance, long nowMs) {
    ...
    scoreEma = (scoreEma < 0.0) ? score : SCORE_EMA_ALPHA * score + (1.0 - SCORE_EMA_ALPHA) * scoreEma;
    return scoreEma;
  }
   ...
   * 0..SEGMENT_COUNT}). Any non-zero score lights at least one segment so the indicator never
   * looks "off" while measuring.
   */
