 ...
 * provides various user interface actions for managing the scan, such as renaming, deleting,
 * sharing, or performing actions with export files. It uses a scanned entity as its core data model
 * and interacts with the scan repository and UI components to update and present the scan's state.
 *
 * <p>Fields: - progress: View that represents the loading progress indicator. - content: View that
 * contains the main content of the fragment. - titleView: TextView that displays the title of the
 * scan. - subtitleView: TextView that displays additional information about the scan. -
 * buttonRename: Button that triggers the rename action for the scan. - buttonDelete: Button that
 * triggers the delete action for the scan. - buttonShare: Button to share the scan's export
 * file(s). - buttonOpenInExport: Button to open the primary export file of the scan in an external
 * app. - buttonAddToCollection: Button to add the scan to a collection. - scanId: String that
 * uniquely identifies the scan being displayed. - entity: The ScanEntity object representing
 * metadata and details of the scan.
 *
 * <p>Methods: - onCreateView: Called to create and initialize the fragment's view hierarchy.
 * Inflates the layout and initializes view components and actions. - openCollectionPickerForScan:
 * Opens a dialog or UI to select a collection to add the specified scan. - loadAsync: Loads the
 * scan details asynchronously and binds the data to the UI components. - bind: Binds a ScanEntity
 * object to the corresponding UI elements of the fragment. - getPrimaryExportUri: Retrieves the
 * primary export file URI for the scan, if available. - showLoading: Toggles the visibility of the
 * loading progress indicator. - showRenameDialog: Displays a dialog allowing the user to rename the
 * scan. - confirmDelete: Shows a confirmation dialog before deleting the scan. - doDelete: Deletes
 * the scan and updates the UI. - share: Shares the scan's export file using the Android sharing
...
public class ScanDetailsFragment extends Fragment {
  ...
  public View onCreateView(
    ...
    restoreAccessLauncher =
        registerForActivityResult(
            new androidx.activity.result.contract.ActivityResultContracts.OpenDocument(),
            uri -> {
              if (uri == null) return;
              // Try to persist read (and write) permission so the file remains accessible across
              // restarts
              try {
                requireContext()
                    .getContentResolver()
    ...
    // Apply system insets: place header just below the status bar and lift the bottom container
    // above the nav bar
    ...
    androidx.core.view.ViewCompat.setOnApplyWindowInsetsListener(
        ...
        (v, insets) -> {
          ...
          if (content != null
              && content.getLayoutParams() instanceof ViewGroup.MarginLayoutParams clp) {
    ...
    if (buttonAddToCollection != null) {
      buttonAddToCollection.setOnClickListener(
          v -> {
  ...
  private void openCollectionPickerForScan(@NonNull String scanId) {
    ...
    new Thread(
            () -> {
              ...
              java.util.List<de.schliweb.makeacopy.data.library.CollectionEntity> filtered =
              ...
              final java.util.List<de.schliweb.makeacopy.data.library.CollectionEntity> finalCols =
                  filtered;
              if (!isAdded()) return;
              requireActivity()
                  .runOnUiThread(
                                  ...
                                  .setItems(
                                      ...
                                      (dialog, which) -> {
                                        if (which == n) {
                                          ...
                                          final de.schliweb.makeacopy.data.library.CollectionEntity
                                              sel = finalCols.get(which);
                                          new Thread(
                                                  () -> {
                                                    try {
  ...
  private android.graphics.Bitmap renderPdfFirstPage(@NonNull android.net.Uri uri, int targetW) {
    ...
    try {
      ...
      try {
        ...
        int targetH =
            (pageW > 0) ? Math.max(1, (int) (targetW * (pageH / (float) pageW))) : targetW;
  ...
  /**
   * Derives a human-friendly parent folder path for the given primary export Uri. Attempts several
   * strategies depending on the URI scheme/provider and falls back gracefully.
   */
      ...
      try {
        if (android.provider.DocumentsContract.isDocumentUri(ctx, uri)) {
          ...
          if (docId != null) {
            ...
            String pathPart =
                colon >= 0
                    ? docId.substring(colon + 1)
                    : docId; // e.g., Documents/MakeACopy/Scan.pdf
                ...
                requireActivity()
                    .runOnUiThread(
                        () -> {
                          ...
                          String fallback =
                              (entity != null
                                      && entity.coverPath != null
                                      && !entity.coverPath.isEmpty())
                                  ? entity.coverPath
                                  : FileUtils.firstUriFromJson(
                                      entity != null ? entity.exportPathsJson : null);
